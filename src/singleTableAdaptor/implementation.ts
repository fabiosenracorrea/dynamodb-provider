...
export class SingleTableProvider implements SingleTableAdaptor {
  ...
  constructor({ databaseProvider }: Params = {}) {
    this.databaseProvider = databaseProvider || new DynamodbProvider();

    this.config = singleTableConfig;
  }
    ...
    await this.databaseProvider.executeTransaction(
      (configs.filter(Boolean) as SingleTableTransactionConfig[]).map(
        ({ create, erase, update, validate }) => {
          if (erase)
            return { erase: { ...this.getDeleteParams(erase), importance: erase.importance } };

          if (create)
            return { create: { ...this.getCreateParams(create), importance: create.importance } };

          if (update)
            return { update: { ...this.getUpdateParams(update), importance: update.importance } };
  ...
  createSetEntity(items: string[]): DBSet {
    return this.databaseProvider.createSetEntity(items);
  }
