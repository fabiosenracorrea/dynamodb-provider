...
export function findOperation(target: OperationTarget, operation: string): Operation | undefined {
  return registry.get(`${target}:${operation}`);
}

export function operationsFor(target: OperationTarget): string[] {
  return ALL.filter((operation) => operation.target === target).map(({ operation }) => operation);
}
